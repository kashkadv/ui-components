...
function CartButton({ label = 'Add to cart' }) { // TODO remove label and direction
  return (
    <Button type='primary' className={`group relative [&>*]:transition-all [&>*]:duration-500 gap-4 px-4 py-2 h-8 items-center flex text-greyDark z-0`}>
      ...
      <div className={`h-[2px] w-1/2 bg-green absolute -bottom-1 transform group-hover:w-full group-hover:h-10 -z-0 ltr:right-5 group-hover:ltr:right-0 rtl:left-4 group-hover:rtl:left-0`}></div> 
    </Button>
  )
}
