...
function ProductInfoNavigation() {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {info.map((feature, i) => (
        <DefaultButton onClick={() => openAccordionTab(i, true)} key={`info-nav-${i}`} className="py-2 text-body font-medium leading-none items-center tracking-widest text-greyDark group hover:text-grey transition-all duration-500">
